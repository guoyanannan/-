...
for i in os.listdir(outputdir):
    path_file = os.path.join(outputdir,i)
    if os.path.isfile(path_file):
        os.remove(path_file)
#创建一个记录检测结果的txt文件
txt_path = outputdir + "result.txt"
file = open(txt_path,'w')

# for jpgfile in glob.glob(path):
#     portion = os.path.split(jpgfile)
#     file.write(portion[1] + ' detect_result：\n')


class YOLO(object):
    _defaults = {
        "model_path": 'logs/001/ep290-loss9.598-val_loss9.414.h5',
    ...
    def detect_image(self, image,jpgfile):
        ...
        for i, c in reversed(list(enumerate(out_classes))):
            ...
            #天机坐标画坐标
            #label = '{} {:.2f} {}'.format(predicted_class, score,box)
            #对灰度图进行RGB转换,输入的原图为单通道，转化为三通道灰度图
            ...
            #从新定义label以及label_size
            label = '{}\n{}{:.2f}'.format(((left, top), (right, bottom)),predicted_class, score)
            label_size = draw.textsize(label, font)
            print(label)
